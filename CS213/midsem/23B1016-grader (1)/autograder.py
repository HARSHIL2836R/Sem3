...
class Autograder:
    ...
    def compile_internal(self,student, show_error=False, rm_includes= True):
        # ---------------------
        extra_inclues = ""
        ...
        if show_error:
            dump_file = ""
        cmd = f"g++ -Wall -std=c++20 -o {self.tmp_path}/{student} {includes} {files} {dump_file}"
        print(cmd)
        os.system(cmd)

    def compile(self,student, show_error=False):
        self.compile_internal(student, show_error)
        if not self.check_compile_ok(student):
            self.compile_internal(student, show_error, rm_includes = False )

    def compile_all(self):
        Parallel(n_jobs=-1)( delayed(self.compile)(student) for student in tqdm(self.students) )
            ...
            for include in self.allowed_include:
                if not include in line:
                    with open(f'{self.tmp_path}/{student}.policy.txt', 'a') as f:
                        f.write(f'Disalloed includes: {line}\n')
    ...
    def disallowed_functions_check(self,student,lines):
        for line in lines:
            ...
            for f in self.disallowed_functions:
                if f in line:
                    with open(f'{self.tmp_path}/{student}.policy.txt', 'a') as f:
                        f.write(f'Disalloed functions:{line}\n')
    ...
    def run_test( self, student, test, timeout ):
    ...
    def run_tests(self):
        ...
        def runner( task ):
            self.run_test( task[0], task[1], task[2] )
    ...
    def test_ok(self,student,test):
        ...
        expected = os.path.abspath(f"{self.tests_path}/{test}/expected_output.txt")
    ...
    def print_fail(self,student):
        for test in self.tests:
            ...
            expected = f"{self.tests_path}/{test}/expected_output.txt"
    ...
    def create_packages(self):
        # for student in self.students:
        #     self.create_student_package(student)
    ...
    #     #---------------------------------------
    #     if act == 'run'    : self.run_tests   ()
    #     if act == 'compile': self.compile_all ()
    #     if act == 'policy' : self.check_policy()

    #     # --------------------------------------
    #     # Report results
    #     # --------------------------------------
    #     if act == 'result' : return self.report_result ()
    #     if act == 'time'   : return self.report_time   ()
    #     # if act == 'count'  : return self.report_count  ()
        
    #     # --------------------------------------
    #     # Report errors
    #     # --------------------------------------
    #     if act == 'cerror' : return self.report_compile_error()
    #     if act == 'rerror' : return self.report_runtime_error()
    #     if act == 'perror' : return self.report_policy_error ()
